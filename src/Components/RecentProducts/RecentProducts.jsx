...
export default function RecentProducts({ product }) {
  ...
  useEffect(() => {
    if (data && data.data && data.data.some((item) => item.id === id)) {
  ...
  const handleToggleWishList = async () => {
    ...
    try {
      ...
      toast.error(
        "Error: Unable to update your wishList. Please try again later."
      );
            ...
            {!isAdding && (
              <>
                <button
                  ...
                  disabled={isAdding}
                  className={`center w-10 h-10 rounded-full opacity-0 transition-all duration-500 cursor-pointer translate-y-16 group-hover:-translate-y-0 group-hover:opacity-100 ${
                    isInWishList
